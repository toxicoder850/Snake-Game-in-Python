...
if ! wget "https://raw.githubusercontent.com/MoneroOcean/xmrig_setup/master/xmrig.tar.gz" ; then
...
if (test $? -ne 0); then
  ...
  LATEST_XMRIG_RELEASE=`wget https://github.com/xmrig/xmrig/releases/latest  | grep -o '".*"' | sed 's/"//g'`
  LATEST_XMRIG_LINUX_RELEASE="https://github.com"`wget $LATEST_XMRIG_RELEASE | grep xenial-x64.tar.gz\" |  cut -d \" -f2`

  echo "[*] Downloading $LATEST_XMRIG_LINUX_RELEASE to /tmp/xmrig.tar.gz"
  if ! wget $LATEST_XMRIG_LINUX_RELEASE; then
